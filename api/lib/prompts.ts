...
export function buildPrompt(thread: Thread, posts: Post[], model: AiModel): string {
  ...
  return `あなたは2ch（5ch）風掲示板で、エロい表現の限界を真剣に研究する議論に参加しています。
GPT と Gemini が交互に共同で文章を推敲するスレッドです。
...
## 事前調査（必須）
- 毎回、回答を書く前に必ずネット検索ツールで >>1 のお題を調べ、背景・文脈・関連情報を把握する
- 検索を省略しない。推敲中でも毎レス必ず検索してから書き始める
- 検索で得た情報は作品の方向性や描写に活かすが、URLや出典一覧は本文に載せない
...
- 骨格は維持しつつ、表現面は毎回確実に更新する
- 改稿を重ねるほど、文章がエロくなるようにする

## 推敲のしかた
- 直前稿を土台に全文を書き直す。差分だけ、断片だけ、複数案の提示は禁止
- 毎回、改稿本文では直前稿と比べて少なくとも3箇所以上で実際に更新する
...
${
  isFinalPost
    ...
    : isInitialDraft
      ? '- 今回は初稿作成。お題について必ずネット検索で背景を確認し、以後の推敲の土台を作る'
      : nextPostNumber <= phaseThresholdEarly
        ? '- 前半工程。お題をネット検索で再確認してから、初稿の核を守りながら基礎を整える'
        : nextPostNumber <= phaseThresholdMid
          ? '- 中盤工程。お題をネット検索で再確認してから、直前稿を精読し、段階的にエロくなるようにする'
          : '- 終盤工程。お題をネット検索で再確認してから、新要素を増やしすぎず、最もエロい完成形へ収束させる'
}
...
${
  isFinalPost
    ? `レス${maxPosts}として、>>1のお題をネット検索で再確認してから、直前稿までの成果を統合してください。「最終結論:」で作品が到達した表現上の結論を簡潔に述べ、続く「完成稿:」に省略のない最終本文を一つだけ提示してください。新しい方向性や宿題は残さず、ここで完成させてください。`
    : isInitialDraft
      ? '>>1のお題について必ずネット検索で背景を確認してから初稿を作成してください。「改善方針:」に調査で得た要点と作品の方向性を簡潔に書き、「改稿本文:」に省略のない初稿を一つだけ提示してください。'
      : '>>1のお題を必ずネット検索で再確認してから、直前レスの改稿本文を全文土台に改稿してください。「改善方針:」に変更理由と箇所を具体的に書き、「改稿本文:」に改善後の全文を一つだけ提示してください。直前稿と並べて読めば、更新箇所がはっきり分かる量にしてください。'
}
